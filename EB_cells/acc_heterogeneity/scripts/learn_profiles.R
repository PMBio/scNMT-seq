...
dt <- data.table(id=character(dt_N), cell_names=list(), anno=character(dt_N), 
                 weight_dist=numeric(dt_N), sc_NLL=numeric(dt_N), bulk_NLL = numeric(dt_N), 
...
for (i in 1:dt_N){
  ...
  if (!is.null(bulk) & (length(ind) / opts$all_cells < opts$cell_cov_prcg)){
    ...
    uniq_cells <- length(cells)                                # Number of unique covereged cells
...
# Store results
saveRDS(dt, paste0(io$out_dir, "cons_prof_", annos, "_basis", opts$basis_prof$M, "_GpCcov", 
