...
load(paste0(io$data_dir, io$type, "_1k_", annos, ".RData"))
io$data_dir   <- paste0(io$base_dir, "/", io$type, "/parsed/profiles/")

annos              <- "prom_500_500"
opts$basis_prof    <- create_rbf_object(M = 25)  # Parameters of the BPR model
...
opts$gpc_cov       <- 30    # Keep regions with enough GpC coverage
