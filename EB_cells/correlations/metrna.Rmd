...
io$outdir <- "/Users/ricard/NMT-seq/rebuttal/EB/correlations/out/metrna"
...
# cells 
opts$cells <- fread(io$in.sample_metadata, header=T) %>% .[pass_metQC==T & pass_rnaQC==T & type=="NMT",sample]
```
...
if (opts$cor_samples) {
  ...
  fwrite(cor_samples, paste0(io$outdir,"/metrna_cor_samples.tsv"), sep="\t")
  
  if (opts$permutation) {
    cor_samples_perm[,(cols) := round(.SD,3), .SDcols=cols]
    fwrite(cor_samples_perm, paste0(io$outdir,"/metrna_cor_samples_perm.tsv"), sep="\t")
  }
}
```
