...
<!-- io$features.indir <- paste(io$base_dir,"features/filt",sep="/") -->
<!-- anno <- fread(sprintf("%s/active_enhancers.bed",io$features.indir))[,c(1,2,3,5)] %>% setnames(c("chr","start","end","id")) -->
<!-- tmp <- merge(cor_samples[anno=="Active enhancers" & sig==T,c("id","padj_fdr")], -->
<!--       anno, by="id") -->
<!-- fwrite(tmp, "/Users/ricard/test/active_enahcners.txt", sep="\t") -->

<!-- # Collect CGI promoters -->
<!-- tmp <- cor_samples[anno=="CGI promoters" & sig==T,c("id","padj_fdr")] -->
<!-- fwrite(tmp, "/Users/ricard/test/cgi_prom.txt", sep="\t") -->
