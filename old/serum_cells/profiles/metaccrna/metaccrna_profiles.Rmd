---
title: "NMT-seq: profiles of DNA methylation and accessibility and its relationship with gene expression"
output: 
...
The idea here is to show methylation and accessibility values around in some genomic elements such as promoters, but stratifying them according to the corresponding gene expression.
The stratification is quite arbitrary, we could use quartiles but here we looked at the histogram of gene expression values and classified all genes into three groups: low, intermediate and high expression.
For genomic contexts that are not directly linked to a gene (such as enhancers) we take a window of predefined length and we associate the element with the corresponding gene expression.
...
<!-- loadMultipleMetData <- function(indir, cells) { -->
<!--   data_list <- list() -->
<!--   for (cell in cells) { -->
<!--     if (file.exists(sprintf("%s/%s.tsv.gz",indir,cell))) { -->
<!--       data_list[[cell]] <- fread(sprintf("zcat < %s/%s.tsv.gz",indir,cell), sep="\t", verbose=F, showProgress=F) %>% -->
...
opts$gene_window <- 5000  # for the overlap between genes and non-genic genomic contexts
...
<!-- Split features in each group by large, medium and low expression -->
