---
title: "NMT-Seq 2i and serum: pseudotime-sorted correlations"
output: 
...
metrna <- fread("/Users/ricard/NMT-seq/correlations/metrna/out/metrna_cor_features.tsv", header=T) %>%
  .[,type:="CG"]

# Load accessibility/RNA correlations
accrna <- fread("/Users/ricard/NMT-seq/correlations/accrna/out/accrna_cor_features.tsv", header=T) %>%
  .[,type:="GC"]

# Load accessibility/methylation correlations
metacc <- fread("/Users/ricard/NMT-seq/correlations/metacc/out/metacc_cor_features.tsv", header=T) %>%
  .[,type:="CGC"]

# Load pseudotime trajectory
pseudotime <- fread("/Users/ricard/NMT-seq/pseudotime/out/destiny.tsv", header=T ) # destiny
...
# pdf("/Users/ricard/NMT-seq/rebuttal/2i_serum/pseudotime/out/metacc_correlation.pdf", width = 8, height = 6)
# print(p)
# dev.off()
```
...
opts$metacc_annos <- c("Exons","CTCF","Nanog","non-CGI promoters","Oct4","p300","Active enhancers")
...
for (a in opts$metacc_annos) {
  p <- ggplot(tmp[type=="CGC" & anno==a], aes(x=pseudotime, y=r, color=anno)) +
    ...
    labs(x="", y="", color="") +
    coord_cartesian(ylim=c(-0.51,0.1)) +
    guides(fill=FALSE) +
    theme_bw() + theme_pub()
  pdf(paste0("/Users/ricard/NMT-seq/pseudotime/out/metacc_",a,".pdf"), width = 4, height = 4, useDingbats = F)
  print(p)
  dev.off()
}
