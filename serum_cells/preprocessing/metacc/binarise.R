...
if (grepl("ricard",Sys.info()['nodename'])) {
  if (opts$context=="CG") {
    io$indir <- "/Users/ricard/data/NMT-seq_EB/met/raw/merged"
    io$outdir <- "/Users/ricard/data/NMT-seq_EB/met/raw/merged/binarised"
  } else {
    io$indir <- "/Users/ricard/data/NMT-seq_EB/acc/raw/merged"
    io$outdir <- "/Users/ricard/data/NMT-seq_EB/acc/raw/merged/binarised"
  }  
} else {
  if (opts$context=="CG") {
    io$indir <- "/hps/nobackup/stegle/users/ricard/NMT-seq/met/raw/allele_unspecific/filtered/unstranded"
    io$outdir <- "/hps/nobackup/stegle/users/ricard/NMT-seq/met/raw/allele_unspecific/filtered/unstranded/binarised"
  } else {
    io$indir <- "/hps/nobackup/stegle/users/ricard/NMT-seq/acc/raw/allele_unspecific/filtered/unstranded"
    io$outdir <- "/hps/nobackup/stegle/users/ricard/NMT-seq/acc/raw/allele_unspecific/filtered/unstranded/binarised"
  }
}
...
for (i in 1:length(samples)) {
  ...
  if (file.exists(outfile)) {
    ...
    cat(sprintf("%s: There are %0.03f%% of sites with non-binary methylation rate\n", samples[i], mean(!data$rate %in% c(0,100))))
    data[,rate:=round(rate/100)*100]
    
    # Save results
    fwrite(data, file=outfile, sep="\t", showProgress=FALSE, verbose=FALSE, col.names=FALSE)
  }
}
